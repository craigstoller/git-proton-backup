...
**Listing is not immediately consistent, and `info` is the authority** (probe **C15**). Immediately after a `trash`, `list` was observed still returning the trashed node while `info` on the same path reported it absent. That is reachable today, not only in Stage 3: a `push :ref` followed by a fresh push can make `ListRefs` name a ref whose node is gone, and `Bootstrap`'s emptiness test has the mirror exposure. Both fail closed and both self-heal on retry. **The untested direction is the dangerous one** — nothing establishes whether `list` can *omit* a node that exists, which is what would silently truncate fetch discovery. The Fetch cache below is keyed on pack name and existence and therefore rests on exactly that unmeasured assumption; probe it before Stage 3 relies on it.
